...
fluentQ was born out of the need to maintain HQL queries in a type-safe way.
...
HQL for Hibernate was the first target language for fluentQ. Today the
...
**Type safety** is the core principle of fluentQ. Queries are constructed
...
To get an impression of the expressivity of the fluentQ query and expression
types, explore the Javadocs for `fluentq.core.Query`,
`fluentq.core.Fetchable`, and `fluentq.core.types.Expression`.
