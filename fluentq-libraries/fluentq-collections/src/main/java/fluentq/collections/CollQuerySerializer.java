 ...
 * @author tiwe
 */
public class CollQuerySerializer extends SerializerBase<CollQuerySerializer> {
  ...
  protected void visitOperation(
    ...
    if (Ops.aggOps.contains(operator)) {
      throw new UnsupportedOperationException(
          "Aggregation operators are only supported as single expressions");
    }
    if (args.size() == 2
        && OPERATOR_SYMBOLS.containsKey(operator)
        && isPrimitiveOrWrapperType(args.get(0).getType())
        && isPrimitiveOrWrapperType(args.get(1).getType())) {
      handle(args.get(0));
