...
import fluentq.codegen.utils.{CodeWriter, ScalaWriter}
import fluentq.codegen._
import fluentq.scala.ScalaEntitySerializer
import fluentq.sql._
import fluentq.sql.codegen._
import fluentq.sql.codegen.support._
...
class ScalaMetaDataSerializer @Inject() (typeMappings: TypeMappings, val namingStrategy: NamingStrategy)
  ...
  override def writeAdditionalProperties(model: EntityType, writer: ScalaWriter): Unit = {
    ...
    if (primaryKeys != null) serializePrimaryKeys(model, writer, primaryKeys.asScala)

    // foreign keys
    val foreignKeys: Collection[ForeignKeyData] =
      model.getData.get(classOf[ForeignKeyData]).asInstanceOf[Collection[ForeignKeyData]]
    if (foreignKeys != null) serializeForeignKeys(model, writer, foreignKeys.asScala, false)
    ...
    if (inverseForeignKeys != null) serializeForeignKeys(model, writer, inverseForeignKeys.asScala, true)
  ...
  def serializeForeignKeys(model: EntityType, writer: CodeWriter,
      foreignKeys: Iterable[_ <: KeyData], inverse: Boolean): Unit = {
    for (fk <- foreignKeys) {
      ...
      if (fk.getForeignColumns.size == 1) {
        ...
        val local = fk.getForeignColumns.asScala.map(c => escape(namingStrategy.getPropertyName(c, model))).mkString(", ")
        val foreign = fk.getParentColumns.asScala.map("\"" + _ + "\"").mkString(", ")
