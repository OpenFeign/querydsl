...
import fluentq.core.group.GroupByBuilder;
...
public class GuavaGroupByBuilder<K> extends GroupByBuilder<K> {
  ...
  public <V extends Comparable<? super V>>
      ResultTransformer<TreeMultimap<K, V>> asSortedSetMultimap(Expression<V> expression) {
    final Expression<V> lookup = getLookup(expression);
    return new GroupByMultimap<>(key, expression) {
      @Override
   ...
   * @return new result transformer
   */
  public <V> ResultTransformer<TreeMultimap<K, V>> asSortedSetMultimap(
   ...
   * @return new result transformer
   */
  public <C, V> ResultTransformer<Table<K, C, V>> asTable(
      final Expression<C> column, final Expression<V> expression) {
   ...
   * @return new result transformer
   */
  public <C extends Comparable<? super C>, V>
      ResultTransformer<TreeBasedTable<K, C, V>> asSortedTable(
          final Expression<C> column, final Expression<V> expression) {
    final Expression<C> columnKeyLookup = getLookup(column);
    final Expression<V> lookup = getLookup(expression);
    return new GroupByTable<>(key, column, expression) {
   ...
   * @return new result transformer
   */
  public <C, V> ResultTransformer<TreeBasedTable<K, C, V>> asSortedTable(
      final Expression<C> column,
