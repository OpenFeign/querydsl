...
import fluentq.core.dml.ReactiveInsertClause;
...
import fluentq.r2dbc.R2dbcUtils;
import fluentq.r2dbc.SQLSerializer;
import fluentq.r2dbc.binding.BindTarget;
import fluentq.r2dbc.binding.StatementWrapper;
import fluentq.r2dbc.types.Null;
import fluentq.sql.ColumnMetadata;
import fluentq.sql.RelationalPath;
import fluentq.sql.SQLBindings;
import fluentq.sql.dml.DefaultMapper;
import fluentq.sql.dml.Mapper;
...
public abstract class AbstractR2DBCInsertClause<C extends AbstractR2DBCInsertClause<C>>
    extends AbstractR2DBCClause<C> implements ReactiveInsertClause<C> {
   ...
   */
  protected <T> Mono<T> executeStatementWithKey(
      Statement stmt, Class<T> type, @Nullable Path<T> path) {
    RowMapper<T> mapper =
        (row, metadata) -> Objects.requireNonNull(row.get(0, type), "Null key result");
    return Mono.from(stmt.execute()).flatMap(result -> Mono.from(result.map(mapper::map)));
  }

  protected <T> Flux<T> executeStatementWithKeys(
      Statement stmt, Class<T> type, @Nullable Path<T> path) {
    RowMapper<T> mapper =
        (row, metadata) -> Objects.requireNonNull(row.get(0, type), "Null key result");
    return Flux.from(stmt.execute()).flatMap(result -> result.map(mapper::map));
  }
