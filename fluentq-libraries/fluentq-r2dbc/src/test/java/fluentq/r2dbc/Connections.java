...
import fluentq.r2dbc.binding.BindTarget;
import fluentq.r2dbc.binding.StatementWrapper;
...
public final class Connections {
  ...
  public static R2DBCConnectionProvider getMySQL() {
    var url = "r2dbc:mysql://fluentq:fluentq@localhost:3306/fluentq?useLegacyDatetimeCode=false";
    return getR2DBCConnectionProvider(url);
  }

  public static R2DBCConnectionProvider getPostgreSQL() {
    var url = "r2dbc:postgresql://fluentq:fluentq@localhost:5433/fluentq";
    return getR2DBCConnectionProvider(url);
  }
  ...
  public static void initH2() {
    ...
    Flux<Void> setup =
        Flux.concat(
            ...
            //                execute(connection, "CREATE ALIAS InitGeoDB for
            // geodb.GeoDB.InitGeoDB").then(),
            //                execute(connection, "CALL InitGeoDB()").then(),
            dropTable(templates, "SHAPES").then(),
            execute(connection, "create table SHAPES (ID int not null primary key, GEOMETRY blob)")
                .then(),
            execute(connection, "drop table QTEST if exists").then(),
            execute(
                    connection,
                    """
                    create table QTEST (ID INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  C1\
                     int NULL)\
                    """)
                .then(),
            ...
            //                execute(connection, "alter table SURVEY alter column id int
            // auto_increment").then(),
    ...
    //            setup = setup.concatWith(execute(connection, "insert into SHAPES values(" +
    // entry.getKey()
    ...
    Flux<Void> setup =
        Flux.concat(
            ...
            execute(
                    connection,
                    """
                    create table SURVEY(ID int primary key auto_increment, NAME varchar(30), NAME2\
                     varchar(30))\
                    """)
                .then(),
            execute(connection, "insert into SURVEY values (1,'Hello World','Hello');").then(),
            ...
            execute(
                    connection,
                    """
                    create table EMPLOYEE ( ID INT PRIMARY KEY AUTO_INCREMENT, FIRSTNAME\
                     VARCHAR(50), LASTNAME VARCHAR(50), SALARY DECIMAL, DATEFIELD DATE,\
                     TIMEFIELD TIME, SUPERIOR_ID INT, CONSTRAINT FK_SUPERIOR FOREIGN\
                     KEY(SUPERIOR_ID) REFERENCES EMPLOYEE(ID) )\
                    """)
                .then(),
            addEmployees(connection, INSERT_INTO_EMPLOYEE).then(),
  ...
  public static void initPostgreSQL() {
    ...
    Flux<Void> setup =
        Flux.concat(
            ...
            execute(connection, "select AddGeometryColumn('SHAPES', 'GEOMETRY', -1, 'GEOMETRY', 2)")
                .then(),
            dropType(connection, "u_country").then(),
            execute(connection, "create type u_country as enum ('Brazil', 'England', 'Germany')")
                .then(),
            dropType(connection, "u_street_type").then(),
            execute(
                    connection,
                    "create type u_street_type as (street VARCHAR(100), number VARCHAR(30))")
                .then(),
            ...
            addEmployees(
                    ...
                    insert into "EMPLOYEE" ("ID", "FIRSTNAME", "LASTNAME", "SALARY",\
                     "DATEFIELD", "TIMEFIELD", "SUPERIOR_ID") values (?,?,?,?,?,?,?)\
                    """)
                .then(),
    ...
    Flux<Void> setup =
        Flux.concat(
            ...
            execute(
                ...
                .then(),
            dropTable(templates, "SURVEY").then(),
            execute(connection, "create table SURVEY(ID int, NAME varchar(30), NAME2 varchar(30))")
                .then(),
            execute(connection, "insert into SURVEY values (1, 'Hello World', 'Hello')").then(),
            dropTable(templates, "TEST").then(),
            execute(connection, CREATE_TABLE_TEST).then(),
  ...
  static Mono<Void> addEmployee(
    ...
    configuration.set(
        bindMarkers.next(), bindTarget, QEmployee.employee.datefield, Constants.localDate);
    configuration.set(
        bindMarkers.next(), bindTarget, QEmployee.employee.timefield, Constants.localTime);
    if (superiorId <= 0) {
