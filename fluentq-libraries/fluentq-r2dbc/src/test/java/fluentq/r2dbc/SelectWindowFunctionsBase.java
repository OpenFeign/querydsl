...
public abstract class SelectWindowFunctionsBase extends AbstractBaseTest {
  ...
  public void windowFunctions() {
    ...
    for (WindowOver<?> wo : exprs) {
      query()
          .from(survey)
          .select(wo.over().partitionBy(survey.name).orderBy(survey.id))
          .fetch()
          .collectList()
          .block();
    }
  ...
  public void windowFunctions_manual_paging() {
    ...
    for (Tuple row :
        query()
            .from(employee)
            .select(employee.firstname, employee.lastname, rowNumber)
            .fetch()
            .collectList()
            .block()) {
    ...
    for (Object[] row :
        query().from(sub2.as(subAlias2)).select(all).fetch().collectList().block()) {
      System.out.println(Arrays.asList(row));
    }
    System.out.println();
  ...
  public void windowFunctions_over() {
    ...
    query()
        .from(employee)
        .select(employee.id, R2DBCExpressions.sum(employee.salary).over())
        .fetch()
        .collectList()
        .block();
  }
  ...
  public void windowFunctions_partitionBy() {
    ...
    query()
        ...
        .select(
            employee.id,
            employee.superiorId,
            R2DBCExpressions.sum(employee.salary).over().partitionBy(employee.superiorId))
        .fetch()
        .collectList()
        .block();
  }
