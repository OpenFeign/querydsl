...
public class ProjectionsFactory {
  ...
  public ProjectionsFactory(FluentQModule module, Target target) {
    this.module = module;
    this.target = target;
  }
  ...
  public <A extends Comparable> Collection<Expression<?>> date(
    ...
    if (module != FluentQModule.COLLECTIONS && module != FluentQModule.RDFBEAN) {
      rv.add(expr.min());
      rv.add(expr.max());
    }
  ...
  private <A extends Number & Comparable<A>> Collection<NumberExpression<?>> numeric(
    ...
    if (!forFilter && module != FluentQModule.COLLECTIONS && module != FluentQModule.RDFBEAN) {
      rv.add(expr.min());
      rv.add(expr.max());
      rv.add(expr.avg());
      rv.add(expr.count());
    ...
    if (!(other instanceof Constant<?> || module == FluentQModule.RDFBEAN)) {
