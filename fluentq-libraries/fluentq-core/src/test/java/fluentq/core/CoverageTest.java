...
import fluentq.core.alias.Alias;
...
class CoverageTest {
  ...
  private MatchingFiltersFactory matchers =
      new MatchingFiltersFactory(FluentQModule.COLLECTIONS, Target.MEM);

  private ProjectionsFactory projections =
      new ProjectionsFactory(FluentQModule.COLLECTIONS, Target.MEM);

  private FilterFactory filters =
      new FilterFactory(projections, FluentQModule.COLLECTIONS, Target.MEM);
  ...
  void test() throws Exception {
    ...
    // dateTime
    exprs.addAll(
        projections.dateTime(
            $(entity.getDateTime()), $(entity.getDateTime()), new java.util.Date(0)));
    exprs.addAll(
        matchers.dateTime($(entity.getDateTime()), $(entity.getDateTime()), new java.util.Date(0)));
    exprs.addAll(
        filters.dateTime($(entity.getDateTime()), $(entity.getDateTime()), new java.util.Date(0)));
    // time
