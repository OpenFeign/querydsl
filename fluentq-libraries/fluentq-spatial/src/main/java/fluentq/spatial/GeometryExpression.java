...
public abstract class GeometryExpression<T extends Geometry> extends SimpleExpression<T> {
  ...
  @Nullable
  private transient volatile NumberExpression<Integer> dimension,
      coordinateDimension,
      spatialDimension,
      srid;

  @Nullable private transient volatile StringExpression geometryType;

  @Nullable private transient volatile StringExpression text;

  @Nullable private transient volatile GeometryExpression<Geometry> envelope, boundary, convexHull;
  @Nullable private transient volatile BooleanExpression empty, simple, threed, measured;
   ...
   * coordinate dimension. In non-homogeneous collections, this will return the largest topological
  ...
  /**
   * Returns the name of the instantiable subtype of Geometry of which this geometric object is an
   * instantiable member. The name of the subtype of Geometry is returned as a string.
   *
  ...
  /**
   * The minimum bounding box for this Geometry, returned as a Geometry. The polygon is defined by
   * the corner points of the bounding box [(MINX, MINY), (MAXX, MINY), (MAXX, MAXY), (MINX, MAXY),
   * (MINX, MINY)]. Minimums for Z and M may be added. The simplest representation of an Envelope is
   * as two direct positions, one containing all the minimums, and another all the maximums. In some
  ...
  /**
   * Returns 1 (TRUE) if this geometric object is the empty Geometry. If true, then this geometric
   * object represents the empty point set ∅ for the coordinate space.
   *
   * @return empty
   */
  ...
  /**
   * Returns 1 (TRUE) if this geometric object has no anomalous geometric points, such as self
   * intersection or self tangency. The description of each instantiable geometric class will
   * include the specific conditions that cause an instance of that class to be classified as not
   * simple.
   *
   * @return simple
   */
  ...
  /* (non-Javadoc)
   * @see fluentq.core.types.dsl.SimpleExpression#eq(java.lang.Object)
   */
  @Override
  ...
  /* (non-Javadoc)
   * @see fluentq.core.types.dsl.SimpleExpression#eq(fluentq.core.types.Expression)
   */
  @Override
   ...
   * for intersections between the interior, boundary and exterior of the two geometric objects as
   * specified by the values in the intersectionPatternMatrix. This returns FALSE if all the tested
  ...
  public BooleanExpression relate(Expression<? extends Geometry> geometry, String matrix) {
    return Expressions.booleanOperation(
        SpatialOps.RELATE, mixin, geometry, ConstantImpl.create(matrix));
  }
   ...
   * in the spatial reference system of this geometric object. Because the geometries are closed, it
   * is possible to find a point on each geometric object involved, such that the distance between
  ...
  /**
   * Returns a geometric object that represents all Points whose distance from this geometric object
   * is less than or equal to distance. Calculations are in the spatial reference system of this
   * geometric object. Because of the limitations of linear interpolation, there will often be some
   * relatively small error in this distance, but it should be near the resolution of the
   * coordinates used.
   *
  ...
  public GeometryExpression<Geometry> buffer(double distance) {
    return GeometryExpressions.geometryOperation(
        SpatialOps.BUFFER, mixin, ConstantImpl.create(distance));
  }

  /**
   * Returns a geometric object that represents the convex hull of this geometric object. Convex
   * hulls, being dependent on straight lines, can be accurately represented in linear
  ...
  /**
   * Returns a geometric object that represents the Point set difference of this geometric object
   * with anotherGeometry.
   *
   * @param geometry other geometry
  ...
  public GeometryExpression<Geometry> transform(int srid) {
    return GeometryExpressions.geometryOperation(
        SpatialOps.TRANSFORM, mixin, ConstantImpl.create(srid));
  }
}
