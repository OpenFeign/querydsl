...
import static fluentq.core.Target.HSQLDB;
...
import static fluentq.jpa.Constants.account;
import static fluentq.jpa.Constants.an;
import static fluentq.jpa.Constants.bar;
import static fluentq.jpa.Constants.calendar;
import static fluentq.jpa.Constants.cat;
import static fluentq.jpa.Constants.catalog;
import static fluentq.jpa.Constants.cust;
import static fluentq.jpa.Constants.fatcat;
import static fluentq.jpa.Constants.foo;
import static fluentq.jpa.Constants.form;
import static fluentq.jpa.Constants.item;
import static fluentq.jpa.Constants.kit;
import static fluentq.jpa.Constants.kitten;
import static fluentq.jpa.Constants.list;
import static fluentq.jpa.Constants.log;
import static fluentq.jpa.Constants.m;
import static fluentq.jpa.Constants.mate;
import static fluentq.jpa.Constants.mother;
import static fluentq.jpa.Constants.n;
import static fluentq.jpa.Constants.name;
import static fluentq.jpa.Constants.offspr;
import static fluentq.jpa.Constants.ord;
import static fluentq.jpa.Constants.p;
import static fluentq.jpa.Constants.param;
import static fluentq.jpa.Constants.payment;
import static fluentq.jpa.Constants.person;
import static fluentq.jpa.Constants.price;
import static fluentq.jpa.Constants.prod;
import static fluentq.jpa.Constants.product;
import static fluentq.jpa.Constants.qat;
import static fluentq.jpa.Constants.rival;
import static fluentq.jpa.Constants.store;
...
import fluentq.jpa.domain.Catalog;
import fluentq.jpa.domain.Color;
import fluentq.jpa.domain.Customer;
import fluentq.jpa.domain.DomesticCat;
import fluentq.jpa.domain.Payment;
import fluentq.jpa.domain.Product;
import fluentq.jpa.domain.QFamily;
...
public class ParsingTest extends AbstractQueryTest {
  ...
  public void docoExamples911() throws Exception {
    query().from(fatcat).where(fatcat.weight.gt(select(cat.weight.avg()).from(cat))).parse();
  }

  @Test
  public void docoExamples911_2() throws Exception {
    query().from(cat).where(cat.name.eqAny(select(name.nickName).from(name))).parse();
  }

  @Test
  public void docoExamples911_3() throws Exception {
    query().from(cat).where(select(mate).from(mate).where(mate.mate.eq(cat)).notExists()).parse();
  }

  @Test
  public void docoExamples911_4() throws Exception {
    query().from(cat).where(selectFrom(mate).where(mate.mate.eq(cat)).exists()).parse();
  }

  @Test
  public void docoExamples911_5() throws Exception {
    query().from(cat).where(cat.name.notIn(select(name.nickName).from(name))).parse();
  }
    ...
    query()
        ...
        .where(
            ord.paid
                ...
                .and(catalog.effectiveDate.gt(DateExpression.currentDate()))
                .and(
  ...
  public void docoExamples94_6() throws Exception {
    query()
        ...
        .select(new QFamily(mother, mate, kitten))
        .parse();
  }
  ...
  public void docoExamples97_7() throws Exception {
    query()
        .from(person)
        .where(person.pid.country.eq("AU"), person.pid.medicareNumber.eq(123456))
        .parse();
  }
  ...
  public void docoExamples97_10() throws Exception {
    query()
        .from(log, payment)
        .where(log.item.instanceOf(Payment.class), log.item.id.eq(payment.id))
        .parse();
  }
  ...
  public void docoExamples97_10_2() throws Exception {
    query()
        .from(log, payment)
        .innerJoin(log.item, item)
        .where(item.instanceOf(Payment.class), item.id.eq(payment.id))
        .parse();
  }
  ...
  public void docoExamples98_9() throws Exception {
    query()
        .from(person, calendar)
        .select(person)
        .where(
            calendar.holidays("national holiday").eq(person.birthDay),
            person.nationality.calendar.eq(calendar))
        .parse();
  }
  ...
  public void docoExamples98_10() throws Exception {
    query()
        .from(item, ord)
        .select(item)
        .where(ord.items(ord.deliveredItemIndices(0)).eq(item), ord.id.eq(1L))
        .parse();
  }
  ...
  public void docoExamples98_11() throws Exception {
    query()
        .from(item, ord)
        .select(item)
        .where(ord.items(ord.items.size().subtract(1)).eq(item))
        .parse();
  }
  ...
  public void docoExamples99() throws Exception {
    query().from(cat).orderBy(cat.name.asc(), cat.weight.desc(), cat.birthdate.asc()).parse();
  }
  ...
  public void where_5() throws Exception {
    query()
        .from(an)
        .where(an.bodyWeight.gt(10), an.bodyWeight.lt(100).or(an.bodyWeight.isNull()))
        .parse();
  }
}
