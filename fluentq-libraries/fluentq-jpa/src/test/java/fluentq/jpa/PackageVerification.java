...
public class PackageVerification {
  ...
  public void verify_package() throws Exception {
    ...
    verify(new File("target/fluentq-jpa-" + version + "-apt-hibernate-one-jar.jar"), true);
    verify(new File("target/fluentq-jpa-" + version + "-apt-one-jar.jar"), false);
  }
  ...
  private void verify(File oneJar, boolean hibernateDeps) throws Exception {
    ...
    Class cl = oneJarClassLoader.loadClass(processor.getName()); // fluentq-apt
