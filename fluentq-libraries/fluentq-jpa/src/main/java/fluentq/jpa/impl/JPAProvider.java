...
import fluentq.jpa.BatooTemplates;
import fluentq.jpa.DataNucleusTemplates;
import fluentq.jpa.EclipseLinkTemplates;
import fluentq.jpa.HQLTemplates;
import fluentq.jpa.Hibernate5Templates;
import fluentq.jpa.JPQLTemplates;
import fluentq.jpa.OpenJPATemplates;
    ...
    for (Map.Entry<Class<?>, JPQLTemplates> entry : mappings.entrySet()) {
      ...
      try {
        if (entityManagerClass.isInstance(em.unwrap(entityManagerClass))) {
          ...
          Exception
              e) { // The PersistenceException is wrapped in an InvocationException for EclipseLink
        // detect by delegate
