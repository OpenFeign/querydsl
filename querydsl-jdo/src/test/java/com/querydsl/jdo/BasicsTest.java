...
public class BasicsTest extends AbstractJDOTest {
  ...
  public void list_distinct() {
    ...
    assertEquals(
        new ArrayList<>(
            query().from(product).orderBy(product.name.asc()).select(product.name).fetch()),
        new ArrayList<>(
            query()
                .from(product)
  ...
  public void factoryExpression_in_groupBy() {
    Expression<Product> productBean =
        Projections.bean(Product.class, product.name, product.description);
  ...
  public void simpleTest() throws IOException {
    ...
    assertEquals(
        "Sony Discman",
        query.from(product).where(product.name.eq("Sony Discman")).select(product.name).fetchOne());
    query.close();
  }
  ...
  public void basicTests() {
    ...
    assertEquals(
        "list", 2, query().from(product).select(product.name, product.description).fetch().size());
