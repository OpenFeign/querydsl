...
public class SubqueriesTest extends AbstractTest {
  ...
  public void test1() {
    assertEquals(
        ...
        serialize(
            select(department)
                .from(department)
                .where(department.employees.size().eq(select(d.employees.size().max()).from(d)))));
  }
  ...
  public void test3() {
    assertEquals(
        "SELECT FROM com.querydsl.jdo.models.company.Employee WHERE this.weeklyhours > (SELECT"
  ...
  public void test4() {
    assertEquals(
        ...
        serialize(
            select(employee)
                .from(employee)
                .where(employee.weeklyhours.gt(select(e.weeklyhours.avg()).from(e)))));
  }
    ...
    assertEquals(
        ...
        serialize(
            select(employee)
                .from(employee, e)
                .where(
                    employee.weeklyhours.eq(e.weeklyhours), employee.firstName.eq("emp1First"))));
  }
}
