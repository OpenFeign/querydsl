...
public class QuerySerializationTest extends AbstractTest {
  ...
  public void selectFrom2Sources() {
    assertEquals(
        ...
        serialize(select(product.name).from(product, other).where(product.name.eq(other.name))));
  }
    ...
    assertEquals(
        ...
        serialize(
            select(product.name)
                ...
                .where(
                    product.price.in(
                        select(other.price).from(other).where(other.name.eq("Some name"))))));
  }
  ...
  public void instanceofQuery() {
    assertEquals(
        ...
        serialize(select(product).from(product).where(product.instanceOf(Book.class))));
  }
}
