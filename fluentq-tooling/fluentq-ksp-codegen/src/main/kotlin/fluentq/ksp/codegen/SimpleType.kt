		...
		private val customTypeMap: Map<ClassName, SimpleType> = mutableMapOf<ClassName, SimpleType>()
			.apply {
				...
				apply {
					...
					for (entry in map) {
						mapping[ClassName("org.geolatte.geom", entry.key)] = ClassName("fluentq.spatial", entry.value)
					}
				}
				...
				apply {
					...
					for (entry in map) {
						mapping[ClassName("com.vividsolutions.jts.geom", entry.key)] = ClassName("fluentq.spatial.jts", entry.value)
						mapping[ClassName("org.locationtech.jts.geom", entry.key)] = ClassName("fluentq.spatial.locationtech.jts", entry.value)
					}
				}
