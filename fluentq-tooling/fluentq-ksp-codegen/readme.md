# KSP for FluentQ
Provides a lightweight and efficient way to generate `Q` classes for FluentQ in Kotlin projects.  
...
dependencies {
	...
	implementation("fluentq:fluentq-jpa:${fluentqVersion}")
	ksp("fluentq:fluentq-ksp-codegen:${fluentqVersion}")
}
```
...
Settings must be prefixed with 'fluentq.'
...
// Example
ksp {
    arg("fluentq.prefix", "QQ")
    arg("fluentq.excludedPackages", "com.example, com.sample")
}
```

## Mixed Java/Kotlin sources

A common reason to use this processor instead of `fluentq-apt` is when the
**entities are Java** but the queries are written in **Kotlin**. With Gradle's
default Java/Kotlin compile order, `fluentq-apt`'s Java Q-classes aren't on
...
[fluentq-examples/fluentq-example-ksp-codegen](../../fluentq-examples/fluentq-example-ksp-codegen).
...
- JPA `@Access(PROPERTY)` (annotations on getters) is **not yet supported** —
  put your JPA annotations on fields. This matches `fluentq-apt`'s default.
- Maven KSP integration is out of scope; `kotlin-maven-plugin` does not natively
  ...
  `fluentq-kotlin-codegen` on Maven.
