/*
 * Copyright 2016, The FluentQ Team (http://www.fluentq.com/team)
 *
...
import static fluentq.apt.APTOptions.FLUENTQ_CREATE_DEFAULT_VARIABLE;
import static fluentq.apt.APTOptions.FLUENTQ_ENTITY_ACCESSORS;
import static fluentq.apt.APTOptions.FLUENTQ_EXCLUDED_CLASSES;
import static fluentq.apt.APTOptions.FLUENTQ_EXCLUDED_PACKAGES;
import static fluentq.apt.APTOptions.FLUENTQ_GENERATED_ANNOTATION_CLASS;
import static fluentq.apt.APTOptions.FLUENTQ_INCLUDED_CLASSES;
import static fluentq.apt.APTOptions.FLUENTQ_INCLUDED_PACKAGES;
import static fluentq.apt.APTOptions.FLUENTQ_LIST_ACCESSORS;
import static fluentq.apt.APTOptions.FLUENTQ_MAP_ACCESSORS;
import static fluentq.apt.APTOptions.FLUENTQ_PACKAGE_SUFFIX;
import static fluentq.apt.APTOptions.FLUENTQ_PREFIX;
import static fluentq.apt.APTOptions.FLUENTQ_SUFFIX;
import static fluentq.apt.APTOptions.FLUENTQ_UNKNOWN_AS_EMBEDDABLE;
import static fluentq.apt.APTOptions.FLUENTQ_USE_FIELDS;
import static fluentq.apt.APTOptions.FLUENTQ_USE_GETTERS;
import static fluentq.apt.APTOptions.FLUENTQ_VARIABLE_NAME_FUNCTION_CLASS;

import fluentq.codegen.CodegenModule;
import fluentq.codegen.DefaultVariableNameFunction;
import fluentq.codegen.EmbeddableSerializer;
import fluentq.codegen.EntitySerializer;
import fluentq.codegen.EntityType;
import fluentq.codegen.Filer;
...
import fluentq.codegen.Serializer;
import fluentq.codegen.SerializerConfig;
import fluentq.codegen.SimpleSerializerConfig;
import fluentq.codegen.SupertypeSerializer;
...
import fluentq.core.util.Annotations;
import fluentq.core.util.StringUtils;
...
public class DefaultConfiguration implements Configuration {
  ...
  @NotNull private final Set<String> excludedPackages, excludedClasses;
  @NotNull private final Set<String> includedPackages, includedClasses;
  @Nullable
  ...
  public DefaultConfiguration(
    ...
    for (Element element : roundEnv.getElementsAnnotatedWith(Config.class)) {
      var fluentqConfig = element.getAnnotation(Config.class);
      var config = SimpleSerializerConfig.getConfig(fluentqConfig);
    ...
    if (options.containsKey(FLUENTQ_ENTITY_ACCESSORS)) {
      entityAccessors = Boolean.parseBoolean(options.get(FLUENTQ_ENTITY_ACCESSORS));
    }
    if (options.containsKey(FLUENTQ_LIST_ACCESSORS)) {
      listAccessors = Boolean.parseBoolean(options.get(FLUENTQ_LIST_ACCESSORS));
    }
    if (options.containsKey(FLUENTQ_MAP_ACCESSORS)) {
      mapAccessors = Boolean.parseBoolean(options.get(FLUENTQ_MAP_ACCESSORS));
    }
    if (options.containsKey(FLUENTQ_CREATE_DEFAULT_VARIABLE)) {
      createDefaultVariable = Boolean.parseBoolean(options.get(FLUENTQ_CREATE_DEFAULT_VARIABLE));
    }
    if (options.containsKey(FLUENTQ_PACKAGE_SUFFIX)) {
      module.bind(
          CodegenModule.PACKAGE_SUFFIX,
          StringUtils.nullToEmpty(options.get(FLUENTQ_PACKAGE_SUFFIX)));
    }
    if (options.containsKey(FLUENTQ_PREFIX)) {
      module.bind(CodegenModule.PREFIX, StringUtils.nullToEmpty(options.get(FLUENTQ_PREFIX)));
    }
    if (options.containsKey(FLUENTQ_SUFFIX)) {
      module.bind(CodegenModule.SUFFIX, StringUtils.nullToEmpty(options.get(FLUENTQ_SUFFIX)));
    }
    if (options.containsKey(FLUENTQ_UNKNOWN_AS_EMBEDDABLE)) {
      unknownAsEmbedded = Boolean.parseBoolean(options.get(FLUENTQ_UNKNOWN_AS_EMBEDDABLE));
    }

    if (options.containsKey(FLUENTQ_EXCLUDED_PACKAGES)) {
      var packageString = options.get(FLUENTQ_EXCLUDED_PACKAGES);
    ...
    if (options.containsKey(FLUENTQ_EXCLUDED_CLASSES)) {
      var classString = options.get(FLUENTQ_EXCLUDED_CLASSES);
    ...
    if (options.containsKey(FLUENTQ_INCLUDED_PACKAGES)) {
      var packageString = options.get(FLUENTQ_INCLUDED_PACKAGES);
      if (!StringUtils.isNullOrEmpty(packageString)) {
        List<String> packages = Arrays.asList(packageString.split(","));
    ...
    if (options.containsKey(FLUENTQ_INCLUDED_CLASSES)) {
      var classString = options.get(FLUENTQ_INCLUDED_CLASSES);
      if (!StringUtils.isNullOrEmpty(classString)) {
        List<String> classes = Arrays.asList(classString.split(","));
    ...
    if (options.containsKey(FLUENTQ_USE_FIELDS)) {
      useFields = Boolean.parseBoolean(options.get(FLUENTQ_USE_FIELDS));
    }

    if (options.containsKey(FLUENTQ_USE_GETTERS)) {
      useGetters = Boolean.parseBoolean(options.get(FLUENTQ_USE_GETTERS));
    }

    if (options.containsKey(FLUENTQ_VARIABLE_NAME_FUNCTION_CLASS)) {
      try {
        @SuppressWarnings("unchecked")
        var variableNameFunctionClass =
            (Class<Function<EntityType, String>>)
                Class.forName(options.get(FLUENTQ_VARIABLE_NAME_FUNCTION_CLASS));
    ...
    Class<? extends Annotation> generatedAnnotationClass =
        GeneratedAnnotationResolver.resolve(options.get(FLUENTQ_GENERATED_ANNOTATION_CLASS));
    module.bindInstance(CodegenModule.GENERATED_ANNOTATION_CLASS, generatedAnnotationClass);

    defaultSerializerConfig =
        new SimpleSerializerConfig(
            entityAccessors, listAccessors, mapAccessors, createDefaultVariable, "");
  }
