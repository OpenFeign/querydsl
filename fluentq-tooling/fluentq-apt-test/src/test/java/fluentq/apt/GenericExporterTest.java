...
import fluentq.apt.domain.AbstractEntityTest;
import fluentq.apt.domain.CustomCollection;
import fluentq.apt.domain.Generic2Test;
import fluentq.apt.hibernate.HibernateAnnotationProcessor;
import fluentq.codegen.GenericExporter;
import fluentq.codegen.Keywords;
import fluentq.codegen.PropertyHandling;
...
public class GenericExporterTest extends AbstractProcessorTest {
  ...
  public void execute() throws IOException {
    // via APT
    process(FluentQAnnotationProcessor.class, CLASSES, "FluentQAnnotationProcessor");
    ...
    execute(expected, "GenericExporterTest", "FluentQAnnotationProcessor");
  }
  ...
  private void execute(List<String> expected, String genericExporterFolder, String aptFolder)
    ...
    var successes = 0;
    for (File file :
        new File("target/" + genericExporterFolder + "/fluentq/apt/domain").listFiles()) {
      var other = new File("target/" + aptFolder + "/fluentq/apt/domain", file.getName());
      if (!other.exists() || !other.isFile()) {
        continue;
      }
      var result1 = Files.readString(file.toPath());
      var result2 = Files.readString(other.toPath());
