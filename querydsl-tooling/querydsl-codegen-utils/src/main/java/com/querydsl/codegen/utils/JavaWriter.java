...
public final class JavaWriter extends AbstractCodeWriter<JavaWriter> {
  ...
  private void annotationConstant(Object value) throws IOException {
    if (value.getClass().isArray()) {
      ...
      if (classes.contains(enumValue.getClass().getName())
        ...
        append(enumValue.getDeclaringClass().getName())
            .append(Symbols.DOT)
            .append(enumValue.name());
      }
  ...
  @Override
  public <T> JavaWriter beginPublicMethod(
      Type returnType,
      String methodName,
  ...
  @Override
  public <T> JavaWriter beginStaticMethod(
      Type returnType,
      String methodName,
      Collection<T> parameters,
      Function<T, Parameter> transformer)
  ...
  public JavaWriter field(Type type, String name) throws IOException {
    return line(
            type.getGenericName(true, packages, classes), Symbols.SPACE, name, Symbols.SEMICOLON)
        .nl();
  }
