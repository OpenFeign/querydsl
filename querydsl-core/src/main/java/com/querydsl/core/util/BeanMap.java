...
public class BeanMap extends AbstractMap<String, Object> implements Cloneable {
   ...
   * data source. This behavior for {@link Map#clear() clear()} differs from the Map contract in
   * that the mappings are not actually removed from the map (the mappings for a BeanMap are fixed).
   */
  @Override
  public void clear() {
    if (bean == null) {
