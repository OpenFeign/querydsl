...
public class PersonDaoImpl implements PersonDao {
  ...
  public Mono<Person> update(Person p) {
    ...
    return queryFactory.update(person).populate(p).where(person.id.eq(id)).execute().map(__ -> p);
  }
  ...
  public Mono<Void> delete(Person p) {
    return queryFactory.delete(person).where(person.id.eq(p.getId())).execute().then();
  }

  private R2DBCQuery<Person> getBaseQuery(Predicate... where) {
    return queryFactory.select(personBean).from(person).where(where);
  }
}
