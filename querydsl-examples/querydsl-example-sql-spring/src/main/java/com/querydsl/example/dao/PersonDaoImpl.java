...
public class PersonDaoImpl implements PersonDao {
  ...
  public Person findById(long id) {
    return queryFactory.select(personBean).from(person).where(person.id.eq(id)).fetchOne();
  }

  @Override
  public List<Person> findAll(Predicate... where) {
    return queryFactory.select(personBean).from(person).where(where).fetch();
  }
  ...
  public Person save(Person p) {
    ...
    if (id == null) {
      ...
      queryFactory.update(person).populate(p).where(person.id.eq(id)).execute();
    }
  ...
  public void delete(Person p) {
    queryFactory.delete(person).where(person.id.eq(p.getId())).execute();
  }
}
