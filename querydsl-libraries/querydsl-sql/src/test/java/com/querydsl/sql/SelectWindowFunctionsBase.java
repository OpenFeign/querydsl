...
public abstract class SelectWindowFunctionsBase extends AbstractBaseTest {
  ...
  public void windowFunctions_manual_paging() {
    Expression<Long> rowNumber =
        SQLExpressions.rowNumber().over().orderBy(employee.lastname.asc()).as("rn");
    var all = Wildcard.all;

    // simple
    System.out.println("#1");
    for (Tuple row :
        query().from(employee).select(employee.firstname, employee.lastname, rowNumber).fetch()) {
    ...
    for (Tuple row :
        query().from(sub3.as(employee2)).select(employee2.firstname, employee2.lastname).fetch()) {
  ...
  public void windowFunctions_over() {
    ...
    query().from(employee).select(employee.id, SQLExpressions.sum(employee.salary).over()).fetch();
  }
  ...
  public void windowFunctions_partitionBy() {
    ...
    query()
        ...
        .select(
            employee.id,
            employee.superiorId,
            SQLExpressions.sum(employee.salary).over().partitionBy(employee.superiorId))
        .fetch();
  }
    ...
    teradataQuery()
        ...
        .qualify(
            SQLExpressions.rank()
                ...
                .orderBy(employee.datefield)
                .eq(1L))
        .select(employee.id, SQLExpressions.sum(employee.salary).over())
        .fetch();
  }
}
