...
public class HibernateQuery<T> extends AbstractHibernateQuery<T, HibernateQuery<T>>
   ...
   */
  public HibernateQuery(Mutiny.Session session) {
   ...
   * @param session session
   */
  public HibernateQuery(Mutiny.Session session, QueryMetadata metadata) {
   ...
   */
  public HibernateQuery(Mutiny.Session session, JPQLTemplates templates) {
   ...
   * @param session session
   */
  public HibernateQuery(Mutiny.StatelessSession session) {
  ...
  protected HibernateQuery<T> clone(SessionHolder sessionHolder) {
    var q = new HibernateQuery<T>(sessionHolder, getTemplates(), getMetadata().clone());
    q.clone(this);
    return q;
  }
  ...
  public <U> HibernateQuery<U> select(Expression<U> expr) {
    ...
    var newType = (HibernateQuery<U>) this;
    return newType;
  }
  ...
  public HibernateQuery<Tuple> select(Expression<?>... exprs) {
    ...
    @SuppressWarnings("unchecked") // This is the new type
    var newType = (HibernateQuery<Tuple>) this;
    return newType;
  }
}
