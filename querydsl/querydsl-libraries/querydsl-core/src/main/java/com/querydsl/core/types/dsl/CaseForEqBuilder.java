...
/**
 * {@code CaseForEqBuilder} enables the construction of typesafe case-when-then-else constructs for
 * equals-operations : e.g.
 *
 * <pre>{@code
...
public class CaseForEqBuilder<D> {
  ...
  private static class CaseElement<D> {
    ...
    @Nullable private final Expression<? extends D> eq;
  ...
  public <T> Cases<T, Expression<T>> then(Expression<T> expr) {
    if (expr instanceof Predicate p) {
      return (Cases) then(p);
  ...
  public <T extends Number & Comparable<?>> Cases<T, NumberExpression<T>> then(
      NumberExpression<T> then) {
    return thenNumber(then);
  }

  public <T extends Number & Comparable<?>> Cases<T, NumberExpression<T>> thenNumber(
      Expression<T> then) {
    type = then.getType();
  ...
  public abstract class Cases<T, Q extends Expression<T>> {
    ...
    public Q otherwise(Expression<T> otherwise) {
      ...
      for (CaseElement<D> element : caseElements) {
        if (last == null) {
          last = Expressions.operation((Class<T>) type, Ops.CASE_EQ_ELSE, element.getTarget());
        } else {
          last =
