...
public abstract class SimpleExpression<T> extends DslExpression<T> {
  ...
  @Nullable private transient volatile NumberExpression<Long> countDistinct;

  @Nullable private transient volatile BooleanExpression isnull, isnotnull;
   ...
   *
   * <p>Use expr.isNull() instead of expr.eq(null)
   *
  ...
  public BooleanExpression in(T... right) {
    if (right.length == 1) {
      ...
      return Expressions.booleanOperation(
          Ops.IN,
          mixin,
          ConstantImpl.create(CollectionUtils.unmodifiableList(Arrays.asList(right))));
    }
  }
  ...
  public BooleanExpression notIn(T... right) {
    if (right.length == 1) {
      ...
      return Expressions.booleanOperation(
          Ops.NOT_IN, mixin, ConstantImpl.create(Arrays.asList(right)));
    }
  }
  ...
  public CaseForEqBuilder<T> when(T other) {
    return new CaseForEqBuilder<>(mixin, ConstantImpl.create(other));
  }
  ...
  public CaseForEqBuilder<T> when(Expression<? extends T> other) {
    return new CaseForEqBuilder<>(mixin, other);
  }
}
