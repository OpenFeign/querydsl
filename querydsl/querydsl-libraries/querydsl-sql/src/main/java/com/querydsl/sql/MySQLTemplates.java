 ...
 *
 * <p>tested with MySQL CE 5.1 and 5.5
 *
 * @author tiwe
 */
@Deprecated
public class MySQLTemplates extends SQLTemplates {
  ...
  protected static final Expression<?> LOCK_IN_SHARE_MODE =
      ExpressionUtils.operation(Object.class, SQLOps.LOCK_IN_SHARE_MODE, Collections.emptyList());
  ...
  public MySQLTemplates(char escape, boolean quote) {
    ...
    add(
        ...
        "str_to_date(date_format({0},'%Y-%m-%d %k:%i'),'%Y-%m-%d %k:%i')");
    add(
        Ops.DateTimeOps.TRUNC_SECOND,
        "str_to_date(date_format({0},'%Y-%m-%d %k:%i:%s'),'%Y-%m-%d %k:%i:%s')");
