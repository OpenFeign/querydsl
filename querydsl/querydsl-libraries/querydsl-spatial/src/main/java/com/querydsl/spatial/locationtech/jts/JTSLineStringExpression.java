...
/**
 * A LineString is a Curve with linear interpolation between Points. Each consecutive pair of Points
...
public abstract class JTSLineStringExpression<T extends LineString> extends JTSCurveExpression<T> {
  ...
  @Nullable private transient volatile NumberExpression<Integer> numPoints;
  ...
  public JTSPointExpression<Point> pointN(int idx) {
    return JTSGeometryExpressions.pointOperation(
        SpatialOps.POINTN, mixin, ConstantImpl.create(idx));
  }
}
