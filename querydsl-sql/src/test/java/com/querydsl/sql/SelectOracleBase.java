...
public abstract class SelectOracleBase extends AbstractBaseTest {
  ...
  public void sumOver() throws SQLException {
    ...
    expectedQuery =
        "select e.LASTNAME, e.SALARY, sum(e.SALARY) over (partition by e.SUPERIOR_ID order by"
            + " e.LASTNAME asc, e.SALARY asc), sum(e.SALARY) over (order by e.SUPERIOR_ID asc,"
            + " e.SALARY asc), sum(e.SALARY) over () from EMPLOYEE e order by e.SALARY asc,"
            + " e.SUPERIOR_ID asc";
