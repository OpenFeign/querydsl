...
public class BigDecimalTest {
  ...
  public void arithmetic() {
    ...
    assertEquals(
        Arrays.asList(BigDecimal.valueOf(11), BigDecimal.valueOf(12)),
        query.<BigDecimal>select(num.add(BigDecimal.TEN)).fetch());
    assertEquals(
        Arrays.asList(BigDecimal.valueOf(-9), BigDecimal.valueOf(-8)),
        query.<BigDecimal>select(num.subtract(BigDecimal.TEN)).fetch());
    assertEquals(
        Arrays.asList(BigDecimal.valueOf(10), BigDecimal.valueOf(20)),
        query.<BigDecimal>select(num.multiply(BigDecimal.TEN)).fetch());
    assertEquals(
        Arrays.asList(new BigDecimal("0.1"), new BigDecimal("0.2")),
