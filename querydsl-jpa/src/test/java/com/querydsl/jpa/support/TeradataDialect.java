...
public class TeradataDialect extends Dialect {
  ...
  public TeradataDialect() {
    ...
    registerFunction(
        "year", new SQLFunctionTemplate(StandardBasicTypes.INTEGER, "extract(year from ?1)"));
    registerFunction(
        "length", new SQLFunctionTemplate(StandardBasicTypes.INTEGER, "character_length(?1)"));
    registerFunction("concat", new VarArgsSQLFunction(StandardBasicTypes.STRING, "(", "||", ")"));
    registerFunction(
        "substring",
        new SQLFunctionTemplate(StandardBasicTypes.STRING, "substring(?1 from ?2 for ?3)"));
    registerFunction(
        "locate", new SQLFunctionTemplate(StandardBasicTypes.STRING, "position(?1 in ?2)"));
    registerFunction("mod", new SQLFunctionTemplate(StandardBasicTypes.STRING, "?1 mod ?2"));
    registerFunction(
        "str", new SQLFunctionTemplate(StandardBasicTypes.STRING, "cast(?1 as varchar(255))"));
    ...
    registerFunction(
        "current_time", new SQLFunctionTemplate(StandardBasicTypes.TIME, "current_time"));
    registerFunction(
        "current_date", new SQLFunctionTemplate(StandardBasicTypes.DATE, "current_date"));
  ...
  /**
   * Get the name of the database type associated with the given {@code java.sql.Types} typecode.
   *
  ...
  public String performDropPreProcess(Statement stmt, String dropSql) throws SQLException {
    ...
    if (alterStr.equalsIgnoreCase(st.nextToken()) && tableStr.equalsIgnoreCase(st.nextToken())) {
      ...
      if (dropStr.equalsIgnoreCase(st.nextToken())
        ...
        String arrStr = null;
        String queryStr =
            "sel IndexId, ChildTable, IndexName from dbc.RI_Distinct_ChildrenV where IndexName = '"
                + constraintName
                + "'";
