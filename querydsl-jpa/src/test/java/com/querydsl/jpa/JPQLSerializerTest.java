...
public class JPQLSerializerTest {
  ...
  public void case1() {
    ...
    assertEquals(
        "case when (cat.toes = ?1) then ?1 when (cat.toes = ?2) then ?2 else ?3 end",
        serializer.toString());
  }
  ...
  public void case1_hibernate() {
    ...
    serializer.handle(expr);
    assertEquals(
        "case when (cat.toes = ?1) then ?1 when (cat.toes = ?2) then ?2 else 4 end",
        serializer.toString());
  }
  ...
  public void case2() {
    ...
    serializer.handle(expr);
    assertEquals(
        "case when (cat.toes = ?1) then (cat.id * ?1) when (cat.toes = ?2) then (cat.id * ?2) else"
            + " ?3 end",
        serializer.toString());
  }
  ...
  public void like() {
    ...
    assertEquals(
        "%abc!!%", serializer.getConstantToAllLabels().keySet().iterator().next().toString());
  }
  ...
  public void substring() {
    ...
    assertEquals(
        "substring(cat.name,length(cat.name) + ?1,?2 - (length(cat.name) - ?2))",
        serializer.toString());
  }
