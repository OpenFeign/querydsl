...
public abstract class AbstractLuceneQuery<T, Q extends AbstractLuceneQuery<T, Q>>
  ...
  public CloseableIterator<T> iterate() {
    ...
    try {
      ...
      if (sort != null) {
        scoreDocs =
            searcher.search(createQuery(), getFilter(), sumOfLimitAndOffset, sort, false, false)
                .scoreDocs;
      } else {
        scoreDocs =
            searcher.search(
                    createQuery(), getFilter(), sumOfLimitAndOffset, Sort.INDEXORDER, false, false)
                .scoreDocs;
      }
  ...
  private T oneResult(boolean unique) {
    try {
      ...
      final ScoreDoc[] scoreDocs =
          searcher.search(createQuery(), getFilter(), maxDoc, Sort.INDEXORDER, false, false)
              .scoreDocs;
      int index = 0;
